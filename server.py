...
if not GEMINI_API_KEY:
    try:
        with open(os.path.join(os.path.dirname(__file__), "api_key.txt"), "r") as f:
            ...
            print("INFO: Loaded API key from api_key.txt", flush=True)
    except:
        GEMINI_API_KEY = None
        print("WARNING: No API key found in environment or api_key.txt!", flush=True)
else:
    print("INFO: Loaded API key from environment variable", flush=True)
